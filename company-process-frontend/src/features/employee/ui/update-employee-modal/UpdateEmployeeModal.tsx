...
export const UpdateEmployeeModal = () => {
	...
	const isActive = useAppSelector(employeeSelections.isActiveUpdateModal);
	const updateEmployeeData = useAppSelector(employeeSelections.updateData);
	...
	const [updateEmployee] = employeeAPI.useUpdateMutation();
	...
	const initialPositionItem = useMemo(() => {
		return positionMenuItems.find((item) => item.value.id === updateEmployeeData.positionId);
	}, [positionMenuItems, updateEmployeeData]);
	...
	const onTogglePermissionView = (key: keyof EmployeePermissions, checked: boolean) => {
		const currentPermissions = updateEmployeeData.permissions ?? {};
	...
	const onUpdate = () => {
		...
		if (!updateEmployeeData.fullName || updateEmployeeData.fullName.trim().length === 0) {
			setIsErrorName(true);
			isError = true;
		}

		if (!updateEmployeeData.email || updateEmployeeData.email.trim().length === 0) {
			setIsErrorEmail(true);
			isError = true;
		}

		if (!updateEmployeeData.birthDate) {
			setIsErrorBirthDate(true);
			isError = true;
		}

		if (!updateEmployeeData.hireDate) {
			setIsErrorHireDate(true);
			isError = true;
		}

		if (!updateEmployeeData.positionId) {
			setIsErrorPositionId(true);
			isError = true;
		}

		const accountLogin = updateEmployeeData.accountLogin?.trim() ?? '';
		const accountPassword = updateEmployeeData.accountPassword?.trim() ?? '';
	...
	const onClose = () => {
		dispatch(employeeActions.setIsActiveUpdateModal(false));
		dispatch(employeeActions.clearUpdateData());
		setIsErrorName(false);
		setIsErrorEmail(false);
		setIsErrorBirthDate(false);
		setIsErrorHireDate(false);
				...
				<div className={styles.mainGrid}>
					<div>
						...
						<input
							...
							value={updateEmployeeData.fullName}
					...
					<div className={styles.positionContainer}>
						...
						<DropDownMenu
							...
							initialSelectedItem={initialPositionItem}
					...
					<div>
						...
						<input
							...
							value={updateEmployeeData.accountLogin ?? ''}
					...
					<div>
						...
						<input
							...
							value={updateEmployeeData.accountPassword ?? ''}
				...
				<div className={styles.permissionsSection}>
					...
					<div className={styles.permissionsTable}>
						{permissionRows.map((row) => {
							const permissions = updateEmployeeData.permissions ?? {};
